...
type AccessSummary = Record<string, {
  role?: string
...
function getAccessSummaryFrom(source: State | Config): AccessSummary {
  ...
  for (const username of usernames) {
    ...
    for (const rc of repositoryCollaborator) {
      ...
      if (permissions.indexOf(rc.permission) < permissions.indexOf(repositories[rc.repository].permission)) {
    ...
    for (const tr of teamRepository) {
      ...
      if (permissions.indexOf(tr.permission) < permissions.indexOf(repositories[tr.repository].permission)) {
...
export async function describeAccessChanges(): Promise<string> {
        ...
        case 'E':
          if (path[1] === 'role') {
            if (change.lhs === undefined) {
              lines.push(`  - will join the organization as a ${change.rhs} (remember to accept the email invitation)`)
            } else if (change.rhs === undefined) {
              lines.push(`  - will leave the organization`)
            } else {
              lines.push(`  - will have the role in the organization change from ${change.lhs} to ${change.rhs}`)
            }
          } else {
            lines.push(`  - will have the permission to ${path[2]} change from ${change.lhs} to ${change.rhs}`)
          }
          break
        case 'N':
  ...
  return changes.length > 0 ? lines.join('\n') : 'There will be no access changes'
}
